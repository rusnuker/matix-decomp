...
public class ModelResolver implements IModelResolver
{
    ...
    public IExpression getExpression(final String name) {
        final ModelVariable modelvariable = this.getModelVariable(name);
        if (modelvariable != null) {
            return modelvariable;
        }
        final IExpression iexpression = this.renderResolver.getParameter(name);
    ...
    @Override
    public ModelVariable getModelVariable(final String name) {
        final String[] astring = Config.tokenize(name, ".");
        if (astring.length != 2) {
            return null;
        }
        ...
        final EnumModelVariable enummodelvariable = EnumModelVariable.parse(s2);
