...
public class GuiIngame extends Gui
{
    ...
    public GuiIngame(final Minecraft mcIn) {
        ...
        final IChatListener ichatlistener = NarratorChatListener.INSTANCE;
        this.chatListeners.get(ChatType.CHAT).add((Object)new NormalChatListener(mcIn));
        this.chatListeners.get(ChatType.CHAT).add(ichatlistener);
        this.chatListeners.get(ChatType.SYSTEM).add((Object)new NormalChatListener(mcIn));
        this.chatListeners.get(ChatType.SYSTEM).add(ichatlistener);
        this.chatListeners.get(ChatType.GAME_INFO).add((Object)new OverlayChatListener(mcIn));
    ...
    private void renderHotbarItem(final int p_184044_1_, final int p_184044_2_, final float p_184044_3_, final EntityPlayer player, final ItemStack stack) {
        if (!stack.isEmpty()) {
            final float f = stack.getAnimationsToGo() - p_184044_3_;
            if (f > 0.0f) {
                GlStateManager.pushMatrix();
    ...
    public void addChatMessage(final ChatType p_191742_1_, final ITextComponent p_191742_2_) {
        for (final IChatListener ichatlistener : this.chatListeners.get(p_191742_1_)) {
