...
public class ParticleManager
{
    ...
    public Particle spawnEffectParticle(final int particleId, final double xCoord, final double yCoord, final double zCoord, final double xSpeed, final double ySpeed, final double zSpeed, final int... parameters) {
        ...
        if (iparticlefactory != null) {
            final Particle particle = iparticlefactory.createParticle(particleId, this.world, xCoord, yCoord, zCoord, xSpeed, ySpeed, zSpeed, parameters);
            if (particle != null) {
            ...
            for (int i = 0; i < 4; ++i) {
                for (int j = 0; j < 4; ++j) {
                    for (int k = 0; k < 4; ++k) {
                        ...
                        this.addEffect(new ParticleDigging(this.world, pos.getX() + d0, pos.getY() + d2, pos.getZ() + d3, d0 - 0.5, d2 - 0.5, d3 - 0.5, state).setBlockPos(pos));
                    }
                }
            }
        }
    ...
    public void addBlockHitEffects(final BlockPos pos, final EnumFacing side) {
        ...
        if (iblockstate.getRenderType() != EnumBlockRenderType.INVISIBLE) {
            ...
            final float f = 0.1f;
            final AxisAlignedBB axisalignedbb = iblockstate.getBoundingBox(this.world, pos);
            ...
            this.addEffect(new ParticleDigging(this.world, d0, d2, d3, 0.0, 0.0, 0.0, iblockstate).setBlockPos(pos).multiplyVelocity(0.2f).multipleParticleScaleBy(0.6f));
        }
    }
    ...
    public void addBlockHitEffects(final BlockPos p_addBlockHitEffects_1_, final RayTraceResult p_addBlockHitEffects_2_) {
        final IBlockState iblockstate = this.world.getBlockState(p_addBlockHitEffects_1_);
        if (iblockstate != null) {
            final boolean flag = Reflector.callBoolean(iblockstate.getBlock(), Reflector.ForgeBlock_addHitEffects, iblockstate, this.world, p_addBlockHitEffects_2_, this);
