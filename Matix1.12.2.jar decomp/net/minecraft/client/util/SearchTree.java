...
public class SearchTree<T> implements ISearchTree<T>
{
    ...
    private void index(final T element) {
        this.idFunc.apply(element).forEach(p_194039_2_ -> this.byName.add((T)element, p_194039_2_.toString().toLowerCase(Locale.ROOT)));
        this.nameFunc.apply(element).forEach(p_194041_2_ -> this.byId.add((T)element, p_194041_2_.toLowerCase(Locale.ROOT)));
    }
    
    @Override
    public List<T> search(final String searchText) {
        final List<T> list = this.byId.search(searchText);
        if (searchText.indexOf(58) < 0) {
            return list;
        }
        final List<T> list2 = this.byName.search(searchText);
