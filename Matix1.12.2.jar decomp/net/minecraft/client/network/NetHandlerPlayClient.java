...
public class NetHandlerPlayClient implements INetHandlerPlayClient
{
    ...
    public void handleSpawnObject(final SPacketSpawnObject packetIn) {
        ...
        if (packetIn.getType() == 10) {
            ...
            entity = new EntityFireworkRocket(this.clientWorldController, d0, d2, d3, ItemStack.EMPTY);
        }
            ...
            entity = new EntityPotion(this.clientWorldController, d0, d2, d3, ItemStack.EMPTY);
            packetIn.setData(0);
        }
    ...
    public void handleEntityVelocity(SPacketEntityVelocity packetIn) {
        final EntityVelocityEvent velocityEvent = new EntityVelocityEvent(packetIn);
    ...
    public void addToSendQueue(final Packet<?> packetIn) {
    ...
    public void handleChat(SPacketChat packetIn) {
        final ReceiveChatEvent chatEvent = new ReceiveChatEvent(packetIn.getChatComponent().getFormattedText());
        EventManager.call(chatEvent);
        if (chatEvent.isCancelled()) {
            return;
        }
        if (!chatEvent.getChatMessage().equals(packetIn.getChatComponent().getFormattedText())) {
            packetIn = new SPacketChat(new TextComponentTranslation(chatEvent.getChatMessage(), new Object[0]), packetIn.getType());
        }
        PacketThreadUtil.checkThreadAndEnqueue((Packet<NetHandlerPlayClient>)packetIn, this, this.gameController);
    ...
    public void handleRecipeBook(final SPacketRecipeBook packetIn) {
        PacketThreadUtil.checkThreadAndEnqueue((Packet<NetHandlerPlayClient>)packetIn, this, this.gameController);
        final RecipeBook recipebook = this.gameController.player.getRecipeBook();
    ...
    public void handleTitle(final SPacketTitle packetIn) {
        ...
        switch (spackettitle$type) {
            ...
            case ACTIONBAR: {
                this.gameController.ingameGUI.setOverlayMessage(s3, false);
                return;
            }
            case RESET: {
            ...
            if (s.startsWith("level://")) {
                try {
                    ...
                    if (file2.isFile()) {
                        ...
                        Futures.addCallback((ListenableFuture)this.gameController.getResourcePackRepository().setServerResourcePack(file2), (FutureCallback)this.createDownloadCallback());
                        return;
                    }
                }
