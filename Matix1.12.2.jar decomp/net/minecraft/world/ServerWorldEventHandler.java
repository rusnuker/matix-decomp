...
public class ServerWorldEventHandler implements IWorldEventListener
{
    ...
    public void playSoundToAllNearExcept(@Nullable final EntityPlayer player, final SoundEvent soundIn, final SoundCategory category, final double x, final double y, final double z, final float volume, final float pitch) {
        this.mcServer.getPlayerList().sendToAllNearExcept(player, x, y, z, (volume > 1.0f) ? ((double)(16.0f * volume)) : 16.0, this.world.provider.getDimensionType().getId(), new SPacketSoundEffect(soundIn, category, x, y, z, volume, pitch));
    }
    ...
    public void playEvent(final EntityPlayer player, final int type, final BlockPos blockPosIn, final int data) {
        this.mcServer.getPlayerList().sendToAllNearExcept(player, blockPosIn.getX(), blockPosIn.getY(), blockPosIn.getZ(), 64.0, this.world.provider.getDimensionType().getId(), new SPacketEffect(type, blockPosIn, data, false));
    }
    ...
    public void sendBlockBreakProgress(final int breakerId, final BlockPos pos, final int progress) {
        for (final EntityPlayerMP entityplayermp : this.mcServer.getPlayerList().getPlayers()) {
