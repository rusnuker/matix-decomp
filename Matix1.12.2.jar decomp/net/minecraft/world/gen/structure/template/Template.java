...
public class Template
{
    ...
    public void addBlocksToWorld(final World worldIn, final BlockPos p_189960_2_, @Nullable final ITemplateProcessor templateProcessor, final PlacementSettings placementIn, final int flags) {
        if ((!this.blocks.isEmpty() || (!placementIn.getIgnoreEntities() && !this.entities.isEmpty())) && this.size.getX() >= 1 && this.size.getY() >= 1 && this.size.getZ() >= 1) {
            ...
            final StructureBoundingBox structureboundingbox = placementIn.getBoundingBox();
            for (final BlockInfo template$blockinfo : this.blocks) {
                final BlockPos blockpos = transformedBlockPos(placementIn, template$blockinfo.pos).add(p_189960_2_);
            ...
            for (final BlockInfo template$blockinfo3 : this.blocks) {
                if (block == null || block != template$blockinfo3.blockState.getBlock()) {
                    final BlockPos blockpos2 = transformedBlockPos(placementIn, template$blockinfo3.pos).add(p_189960_2_);
