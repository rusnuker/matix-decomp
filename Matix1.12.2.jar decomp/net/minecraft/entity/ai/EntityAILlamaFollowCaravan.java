...
public class EntityAILlamaFollowCaravan extends EntityAIBase
{
    ...
    public boolean shouldExecute() {
        ...
        for (final EntityLlama entityllama2 : list) {
            if (entityllama2.inCaravan() && !entityllama2.hasCaravanTrail()) {
                final double d2 = this.llama.getDistanceSqToEntity(entityllama2);
                if (d2 > d0) {
                    continue;
                }
                d0 = d2;
        ...
        if (entityllama == null) {
            for (final EntityLlama entityllama3 : list) {
                if (entityllama3.getLeashed() && !entityllama3.hasCaravanTrail()) {
                    final double d3 = this.llama.getDistanceSqToEntity(entityllama3);
                    if (d3 > d0) {
                        continue;
                    }
                    d0 = d3;
    ...
    public boolean shouldContinueExecuting() {
        if (this.llama.inCaravan() && this.llama.getCaravanHead().isEntityAlive() && this.firstIsLeashed(this.llama, 0)) {
            final double d0 = this.llama.getDistanceSqToEntity(this.llama.getCaravanHead());
            if (d0 > 676.0) {
    ...
    public void updateTask() {
        if (this.llama.inCaravan()) {
            ...
            final double d0 = this.llama.getDistanceToEntity(entityllama);
            final float f = 2.0f;
