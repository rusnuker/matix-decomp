...
public class PlayerChunkMap
{
    ...
    public PlayerChunkMap(final WorldServer serverWorld) {
        ...
        this.dirtyEntries = Sets.newHashSet();
        this.pendingSendToPlayers = Lists.newLinkedList();
        this.entriesWithoutChunks = Lists.newLinkedList();
    ...
    public void tick() {
        ...
        if (i - this.previousTotalWorldTime > 8000L) {
            ...
            for (int j = 0; j < this.entries.size(); ++j) {
                final PlayerChunkMapEntry playerchunkmapentry = this.entries.get(j);
                playerchunkmapentry.update();
                playerchunkmapentry.updateChunkInhabitedTime();
            }
        }
        if (!this.dirtyEntries.isEmpty()) {
            for (final PlayerChunkMapEntry playerchunkmapentry2 : this.dirtyEntries) {
        ...
        if (!this.entriesWithoutChunks.isEmpty()) {
            ...
            int k = 49;
            final Iterator<PlayerChunkMapEntry> iterator = this.entriesWithoutChunks.iterator();
            while (iterator.hasNext()) {
