...
public class NetHandlerPlayServer implements INetHandlerPlayServer, ITickable
{
    ...
    public void processVehicleMove(final CPacketVehicleMove packetIn) {
        ...
        if (isMoveVehiclePacketInvalid(packetIn)) {
            ...
            if (entity != this.player && entity.getControllingPassenger() == this.player && entity == this.lowestRiddenEnt) {
                ...
                this.vehicleFloating = (d12 >= -0.03125 && !this.serverController.isFlightAllowed() && !worldserver.checkBlockCollision(entity.getEntityBoundingBox().grow(0.0625).expand(0.0, -0.55, 0.0)));
    ...
    public void handleSeenAdvancements(final CPacketSeenAdvancements p_194027_1_) {
        ...
        if (p_194027_1_.getAction() == CPacketSeenAdvancements.Action.OPENED_TAB) {
            ...
            final Advancement advancement = this.serverController.getAdvancementManager().getAdvancement(resourcelocation);
            if (advancement != null) {
    ...
    public void processPlayer(final CPacketPlayer packetIn) {
        ...
        if (isMovePlayerPacketInvalid(packetIn)) {
            ...
            final WorldServer worldserver = this.serverController.getWorld(this.player.dimension);
                    ...
                    if (this.player.isRiding()) {
                        ...
                        if (this.player.isPlayerSleeping()) {
                            ...
                            if (!this.player.isInvulnerableDimensionChange() && (!this.player.getServerWorld().getGameRules().getBoolean("disableElytraMovementCheck") || !this.player.isElytraFlying())) {
                                ...
                                if (d12 - d11 > f3 * i && (!this.serverController.isSinglePlayer() || !this.serverController.getServerOwner().equals(this.player.getName()))) {
    ...
    public void processTryUseItemOnBlock(final CPacketPlayerTryUseItemOnBlock packetIn) {
        ...
        if (blockpos.getY() < this.serverController.getBuildLimit() - 1 || (enumfacing != EnumFacing.UP && blockpos.getY() < this.serverController.getBuildLimit())) {
            if (this.targetPos == null && this.player.getDistanceSq(blockpos.getX() + 0.5, blockpos.getY() + 0.5, blockpos.getZ() + 0.5) < 64.0 && !this.serverController.isBlockProtected(worldserver, blockpos, this.player) && worldserver.getWorldBorder().contains(blockpos)) {
    ...
    public void handleSpectate(final CPacketSpectate packetIn) {
        ...
        if (this.player.isSpectator()) {
            Entity entity = null;
            for (final WorldServer worldserver : this.serverController.worlds) {
                if (worldserver != null) {
            ...
            if (entity != null) {
                ...
                if (entity.world == this.player.world) {
                    ...
                    this.serverController.getPlayerList().preparePlayer(this.player, worldserver2);
                    this.player.setPositionAndUpdate(entity.posX, entity.posY, entity.posZ);
                    this.player.interactionManager.setWorld(worldserver3);
                    this.serverController.getPlayerList().updateTimeAndWeatherForPlayer(this.player, worldserver3);
    ...
    public void processTabComplete(final CPacketTabComplete packetIn) {
        ...
        for (final String s : this.serverController.getTabCompletions(this.player, packetIn.getMessage(), packetIn.getTargetBlock(), packetIn.hasTargetBlock())) {
            list.add(s);
        }
