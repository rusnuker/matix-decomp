...
public class Shaders
{
        ...
        for (final Object o : set) {
            ...
            if (s2.startsWith(s)) {
                ...
                if (i < 0) {
                    ...
                    try {
                        final String s5 = "shaders/" + StrUtils.removePrefix(s4, "/");
                        final InputStream inputstream = Shaders.shaderPack.getResourceAsStream(s5);
                        if (inputstream == null) {
                            SMCLog.warning("Texture not found: " + s4);
                        }
                        else {
                            IOUtils.closeQuietly(inputstream);
                            final SimpleShaderTexture simpleshadertexture = new SimpleShaderTexture(s5);
                            simpleshadertexture.loadTexture(Shaders.mc.getResourceManager());
                            final CustomTexture customtexture = new CustomTexture(i, s5, simpleshadertexture);
                            list.add(customtexture);
                        }
                    }
                    catch (final IOException ioexception) {
                        SMCLog.warning("Error loading texture: " + s4);
    ...
    private static void checkWorldChanged(final World world) {
        if (Shaders.currentWorld != world) {
            ...
            if ((Shaders.currentWorld = world) != null && oldWorld != null) {
                final int i = world.provider.getDimensionType().getId();
                final int j = oldWorld.provider.getDimensionType().getId();
    ...
    static {
        Shaders.mc = Wrapper.getMinecraft();
